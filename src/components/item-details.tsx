...
// Description Section
function DescriptionSection({ desc }: { readonly desc: string }) {
  return (
...
function SupportedOSSection({
  bottle,
}: {
  readonly bottle?: FormulaDetails["bottle"];
}) {
  return (
    <section className="rounded-xl shadow-md dark:bg-gray-800 bg-white p-6">
      <h2 className="mb-2 text-xl font-semibold text-gray-900 dark:text-gray-100">
      ...
      <div className="flex flex-wrap items-center gap-3">
        {bottle?.stable?.files ? (
          Object.keys(bottle.stable.files).map((arch) => (
            <span
              key={arch}
...
function InstallCommandSection({ name }: { readonly name: string }) {
  return (
    <section className="rounded-xl shadow-md dark:bg-gray-800 bg-white p-6">
      <h2 className="mb-2 text-xl font-semibold text-gray-900 dark:text-gray-100">
        Install Command
      ...
      <div className="flex items-center gap-2">
        ...
        <button
          ...
          onClick={() => {
            navigator.clipboard.writeText(`brew install ${name}`);
          }}
...
// Versions and Dependencies Section
function VersionsDependenciesSection({
  formula,
}: {
  readonly formula: FormulaDetails;
}) {
  return (
    <section className="rounded-xl shadow-md dark:bg-gray-800 bg-white p-6 flex flex-col gap-3">
      ...
      {formula.build_dependencies && formula.build_dependencies.length > 0 && (
        <div>
          <span className="text-xs text-gray-500 dark:text-gray-400">
            Build dependencies
          </span>
...
function AnalyticsSection({
  ...
  readonly analytics: FormulaDetails["analytics"];
}) {
  if (!analytics?.install) return null;
  return (
    <section className="rounded-xl shadow-md dark:bg-gray-800 bg-white p-6">
